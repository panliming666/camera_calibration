...
//      D[i][j] = d1 * d1 + 10 * d2;
    ...
    GrowType grow_board(const Corner &corners, std::vector<int> &used, Board &board,
        ...
        if (board.idx.empty()) { return GrowType_Failure; }
        ...
        if (params.corner_type == MonkeySaddlePoint) {
            ...
            for (int i = 0; i < proposal.size(); ++i) {
                if (pred[i] == -2) { --board.num; }
                board.idx[proposal[i].y][proposal[i].x] = pred[i];
            }
        }

        // grow inside corners
        if (!proposal.empty()) { return GrowType_Inside; }

        // add proposal top/left/bottom/right
        if (!add_board_boundary(board, direction)) { return GrowType_Failure; }
        p1.clear();
        p2.clear();
        p3.clear();
        ...
        if (proposal.empty()) { return GrowType_Failure; }
